        ...
        <div>
            <div className="flex justify-between items-end mb-8 border-b border-[var(--color-border)] pb-4">
                <div>
                    ...
                    <p className="text-sm text-[var(--color-muted)] mt-1">Manage your portfolio sections.</p>
                </div>
                <form action={async (formData) => {
                    ...
                    <input name="category" type="text" placeholder="New Category (e.g. Maternity)" required className="bg-[var(--color-surface)] border border-[var(--color-border)] px-4 py-2 text-sm text-white focus:outline-none focus:border-gold rounded-sm" />
            ...
            {!success ? (
                <div className="p-4 border border-red-500/30 bg-red-500/10 text-red-500 text-sm rounded-sm">
                    Failed to load categories. Ensure your Cloudinary credentials are correct and you have created a &quot;portfolio&quot; folder.
                </div>
            ) : folders?.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {folders?.map((folder: any) => (
                        <Link key={folder.path} href={`/admin/portfolio/${folder.name}`} className="group block p-6 bg-[var(--color-surface)] border border-[var(--color-border)] hover:border-gold transition-colors rounded-sm cursor-pointer relative overflow-hidden">
